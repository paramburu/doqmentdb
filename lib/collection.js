 ...
 * @returns {*}
 */
Collection.prototype.$bulkCreate = function(docs) {
  var self = this
    , manager = self.manager
    ;

  return using(_findOrCreateSporc.call(self, '$BULK_CREATE')
    , function(sporc) {
        return manager.executeSporc.call(manager, sporc._self, [docs]);
      });
};
 ...
 * @param sporc
 */
Collection.prototype.addSproc =
Collection.prototype.addSporc = function (sporc) {
  this.customSporcs[sporc.id] = sporc;
};
 ...
 * @returns {*}
 */
Collection.prototype.runSproc =
Collection.prototype.runSporc = function () {
  ...
  return using(_findOrCreateSporc.call(self, name)
    , function(sporc) {
        return manager.executeSporc.call(manager, sporc._self, args);
      });
};
...
function _findOrCreateSporc(name) {
  var self = this
    ...
    , sporcRecipe = SPORCS[name] || self.customSporcs[name]
    ;

  return self.sporcs[name] ||
    using(_getCollection.call(self), { id: sporcRecipe.id }, findSporc)
      .then(function(data) {
        return _.isEmpty(data)
          ? manager.createSporc.call(manager, self.coll, sporcRecipe)  // create sporc based const
          : data
      })
      .then(function(sporc) {
...
function _getCollection() {
  var ctx = this
    , manager = ctx.manager
    , db = _.isDataBase(ctx.db) ? ctx.db : manager.findOrCreate({ id: ctx.db });
