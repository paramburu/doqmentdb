...
Manager.prototype.executeSporc = function(self, args) {
  ...
  return promisify(client.executeStoredProcedure)
    ...
    .catch(function (e) { return e.retryMs && e.retryMs > 0; }, function (e) {
        console.log('Oops retry in: ', e.retryMs + 400);

        return Promise.delay(parseInt(e.retryMs + 400)).then(function () {
            console.log('Retrying...');

            return _this.executeSporc(self, args);
        });
    })
    .then(_.first);
};
